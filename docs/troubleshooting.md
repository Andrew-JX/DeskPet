...
| 点击/互动后宠物闪成默认狗(占位) | 待机是视频时，切回视频会先清空画面再异步重载视频，重载期间显示占位；素材读不到也直接闪占位 | 视频改为新视频首帧就绪后才切换、期间保持当前画面(loadToken防竞态)；素材加载失败保持当前画面+打日志，不闪占位 | - |
| 全屏架构下宠物到不了屏幕右/下边缘(空气墙) | 夹边界用小组偏移估算有误差 | 改为先放置再量本体真实rect、越界才回拉，可精确贴各边 | - |
